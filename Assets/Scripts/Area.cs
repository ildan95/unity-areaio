...
public class Area : MonoBehaviour {
	...
	void Awake () {
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            {
                AreaBlock block = Instantiate(blockPrefab, new Vector3(i, 1, j), Quaternion.identity);
    ...
    public static bool isFreeSquare(int x, int z, int size)
    {
        if (x + size >= rows || z + size >= cols)
            return false;
    ...
    public static bool isOut(int x, int z)
    {
        return x > rows-1 || x < 0 || z > cols-1 || z < 0;
    }
}
